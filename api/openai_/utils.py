...
TEMPERATURE = 1
MAX_RETRIES = 1
OPENAI_MODEL = "gpt-3.5-turbo" #for debugging, use the cheaper api
import openai
import time, yaml


def load_client_openai(key_path="openai_config.yaml"):
...
def send_message_openai(messages, client = None, agent_config = {}):
    '''
...
if __name__=="__main__":
    client = load_client_openai("openai_config.yaml")
    prompt="Can you teach me quantum physics?"
    response = send_message_openai(prompt, client)
    print(response)