...
TEMPERATURE = 1
MAX_RETRIES = 1
DEEPSEEK_MODEL = "deepseek-chat" #for debugging, use the cheaper api
import openai
import time, yaml


def load_client_deepseek(key_path="deepseek_config.yaml"):
...
def send_message_deepseek(messages, client = None, agent_config = {}):
    '''
    ...
    model_name = agent_config.get("model", DEEPSEEK_MODEL)
...
if __name__=="__main__":
    client = load_client_deepseek("deepseek_config.yaml")
    prompt="Can you teach me quantum physics?"
    response = send_message_deepseek(prompt, client)
    print(response)